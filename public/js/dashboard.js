...
async function loadDashboard() {
    ...
    const [dash, pl, ytdPL, drops, trend] = await Promise.all([
        authFetch(`${API}/dashboard/stats`).then(r => r.json()).catch(() => ({})),
    ...
    renderDocStats(dash);
    renderPayablesStrip(dash.payables || {});
    renderPLSnapshot(pl, mStart, mEnd, ytdPL, fyStart);
    renderRecentDocs(dash.recent_docs || []);
...
// ── Doc stats ──────────────────────────────────────────────────────────────────
function renderDocStats(dash) {
    const s = dash.stats || {};
    setText('total-docs',   s.total_bills || '—');
    setText('month-docs',   s.bills_this_month || '0');
    setText('pending-docs', s.pending_review || '0');
    const overdue = Number(dash.payables?.overdue_count || 0);
    setHTML('pay-overdue', overdue > 0 ? `<span class="text-red-600 font-bold">${overdue}</span>` : '0');
}

// ── Payables strip ─────────────────────────────────────────────────────────────
function renderPayablesStrip(p) {
    setHTML('pay-week',  fmt(p.due_7d));
    setHTML('pay-month', fmt(p.due_30d));
    setHTML('pay-total', fmt(p.total_outstanding));
}
...
function renderPLSnapshot(d, start, end, ytd, fyStart) {
    ...
    if (!d.success) {
        ['pl-net-sales','pl-gross-profit','pl-cm1','pl-ebitda'].forEach(id => setText(id, '—'));
    ...
    setText('pl-gp-pct',     pct(gp, ns) ? pct(gp, ns) + ' GM' : '');
    setText('pl-cm1-pct',    pct(cm1, ns) ? pct(cm1, ns) + ' CM' : '');
    setText('pl-ebitda-pct', pct(ebitda, ns) ? pct(ebitda, ns) + ' margin' : '');
    ...
    if (totalExp > 0) {
        ...
        if (bar) {
            const p = v => Math.round(v / totalExp * 100);
            bar.innerHTML = `
                <div class="flex rounded-lg overflow-hidden h-2.5 w-full mt-4">
                    <div class="bg-indigo-500" style="width:${p(cogsAmt)}%" title="COGS ${fmt(cogsAmt)}"></div>
                    <div class="bg-blue-400"   style="width:${p(fulAmt)}%"  title="Fulfilment ${fmt(fulAmt)}"></div>
                    <div class="bg-violet-400" style="width:${p(mktAmt)}%"  title="Marketing ${fmt(mktAmt)}"></div>
                    <div class="bg-slate-300"  style="width:${p(opexAmt)}%" title="Operations ${fmt(opexAmt)}"></div>
                </div>
...
function renderYTD(ytd, fyStart) {
    ...
    el.innerHTML = `
        ...
        <div class="flex gap-4 flex-wrap">
            ...
            <div><span class="text-xs text-slate-500">Gross Profit</span><br><span class="text-sm font-bold text-blue-600">${fmt(gp)}</span> <span class="text-xs text-slate-400">${pct(gp,ns)}%</span></div>
            <div><span class="text-xs text-slate-500">CM1</span><br><span class="text-sm font-bold text-violet-600">${fmt(cm1)}</span> <span class="text-xs text-slate-400">${pct(cm1,ns)}%</span></div>
            <div><span class="text-xs text-slate-500">EBITDA</span><br><span class="text-sm font-bold ${eb >= 0 ? 'text-emerald-600' : 'text-red-500'}">${fmt(eb)}</span> <span class="text-xs text-slate-400">${pct(eb,ns)}%</span></div>
        </div>`;
...
function renderRecentDocs(docs) {
    ...
    const STATUS_MAP = {
        uploaded:        ['Pending', 'bg-amber-100 text-amber-700'],
        processed:       ['Done',    'bg-emerald-100 text-emerald-700'],
        manual_required: ['Review',  'bg-orange-100 text-orange-700'],
        error:           ['Error',   'bg-red-100 text-red-600'],
    };
    container.innerHTML = docs.map(doc => {
        const vendor = doc.bill_vendor_name || doc.vendor_name || doc.file_name || '—';
        const grp    = (doc.bill_category_group || '').toUpperCase();
        const amount = doc.bill_total_amount;
        const grpChip = GRP_CHIP[grp] || '';
        const uploadDate = doc.uploaded_at ? new Date(doc.uploaded_at).toLocaleDateString('en-IN', { day:'numeric', month:'short' }) : '';
        ...
        <div class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition cursor-pointer" onclick="location.href='documents.html?doc=${doc.document_id}'">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-1.5 flex-wrap text-xs text-slate-400 mt-0.5">
                    ${grpChip}
                    ${amount ? `<span class="font-semibold text-slate-700">${fmt(amount)}</span>` : ''}
...
function renderMiniTrend(data) {
    ...
    bars.innerHTML = months.map(m => {
        ...
        return `<a href="${link}" class="flex flex-col items-center flex-1 gap-0.5 group" title="${m.label}: ${fmt(m.net_sales)} sales">
...
async function loadWatchdog(silent) {
    ...
    if (!container) return;
    if (!silent) container.innerHTML = `<div class="text-xs text-slate-400 px-1 py-2">Running checks…</div>`;
    try {
        ...
        container.innerHTML = `<div class="text-xs text-red-400 px-1 py-2">Watchdog unavailable</div>`;
    }
}

async function refreshWatchdog() {
    const container = document.getElementById('watchdog-list');
    if (container) container.innerHTML = `<div class="text-xs text-slate-400 px-1 py-2">Re-running…</div>`;
...
function renderWatchdog(data, alerts) {
    ...
    const checks = [
        ...
        { key: 'aged_unpaid',   label: 'Unpaid > 30d',       icon: 'fa-calendar-xmark', href: 'payables.html?filter=overdue',           items: (data.aged_unpaid   || []).slice(0,2).map(b => (b.vendor_name || 'Vendor') + ' · ' + fmt(b.total_amount)) },
        { key: 'uncategorized', label: 'Uncategorised bills', icon: 'fa-tag',            href: 'documents.html?flag=no-group',          items: (data.uncategorized || []).slice(0,2).map(b => (b.vendor_name || 'Vendor') + ' · ' + fmt(b.total_amount)) },
    ];
    ...
    const alertsHtml = alerts.length
        ? alerts.map(a => `<div class="text-xs px-2 py-1.5 rounded-lg ${a.severity === 'critical' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'} mb-1">
            <span class="font-semibold">${(a.alert_type||'').replace(/_/g,' ')}</span> — ${a.message || ''}
          </div>`).join('')
        : `<div class="text-xs text-emerald-600 px-1 py-1">All alerts cleared ✓</div>`;
    container.innerHTML = checksHtml + `<div class="pt-2 mt-1 border-t border-slate-100">${alertsHtml}</div>`;
}
    ...
    try {
        ...
        el.innerHTML = allGood
            ...
            : `<div class="space-y-1.5">
                ${docsUnposted > 0 ? `<a href="quality.html?filter=unposted" class="flex items-center justify-between text-xs py-1.5 px-2 rounded-lg bg-amber-50 hover:bg-amber-100 transition">
                    <span class="text-amber-700"><i class="fas fa-inbox mr-1"></i>${docsUnposted} doc${docsUnposted!==1?'s':''} unposted</span>
                    <span class="font-semibold text-amber-700">${fmt(unpostedAmt)}</span>
                </a>` : ''}
                ${missingCoa > 0 ? `<a href="quality.html" class="flex items-center justify-between text-xs py-1.5 px-2 rounded-lg bg-rose-50 hover:bg-rose-100 transition">
                    <span class="text-rose-600"><i class="fas fa-tag mr-1"></i>Missing COA</span>
                    <span class="font-semibold text-rose-600">${fmt(missingCoa)}</span>
                </a>` : ''}
                ${missingDrop > 0 ? `<a href="quality.html" class="flex items-center justify-between text-xs py-1.5 px-2 rounded-lg bg-rose-50 hover:bg-rose-100 transition">
                    <span class="text-rose-600"><i class="fas fa-layer-group mr-1"></i>Missing drop</span>
